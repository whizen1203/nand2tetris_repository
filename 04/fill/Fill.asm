...
(LOOP)
    ...
    @SETSCREEN
    D;JEQ       // If no key, set screen to zeroes (white) // JEQ == if value and D register's value are zero, jump to destination.
...
(SETLOOP)    
    ...
    M=D         // i=i-1
    @LOOP
    D;JLT       // if i<0 goto LOOP  // JLT == if value is minus, jump. if D register is minus, jump to destination.
